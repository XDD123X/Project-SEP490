...
def process_video(video_path, known_encodings, known_names, seconds_interval=1):
    """
    Mở video và xử lý từng frame theo khoảng thời gian (giây):
      - Tính FPS của video và xử lý 1 frame sau mỗi 'seconds_interval' giây.
    ...
    if fps == 0:
        fps = 30  # Giá trị dự phòng nếu không lấy được fps
    ...
    while True:
        ...
        frame_count += 1
        # Xử lý frame dựa trên khoảng cách frame tính từ số giây mong muốn
...
def main():
    ...
    # Xử lý video: thay vì theo số frame, ta xử lý 1 frame mỗi 'seconds_interval' giây (mặc định 1 giây)
    process_video(video_path, known_encodings, known_names, seconds_interval=1)
