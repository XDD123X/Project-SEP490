...
import { GetLecturerRequests, UpdateLecturerRequest } from "@/services/sessionRequestService";
import { format } from "date-fns";

export default function LecturerRequestManagement() {
  ...
  const fetchData = async () => {
    try {
      const response = await GetLecturerRequests();
  ...
  const filteredAndSortedRequests = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter((req) => req.lecturer.fullName.toLowerCase().includes(query) || req.lecturer.email.toLowerCase().includes(query) || (req.description && req.description.toLowerCase().includes(query)));
    }
  ...
  return (
    <div className="container mx-auto py-8">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold">Lecturer Request Management</CardTitle>
          <CardDescription>Manage lecturer session change requests. You can view, accept, or reject pending requests.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="w-full sm:w-48">
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="1">Accepted</SelectItem> <SelectItem value="2">Rejected</SelectItem>
                  </SelectContent>
                </Select>
              </div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="cursor-pointer" onClick={() => requestSort("lecturer.fullName")}>
                  <div className="flex items-center">
                    Lecturer Name
                    {getSortDirectionIcon("lecturer.fullName")}
                  </div>
                </TableHead>
                <TableHead className="cursor-pointer" onClick={() => requestSort("lecturer.email")}>
                  <div className="flex items-center">
                    Email
                    {getSortDirectionIcon("lecturer.email")}
                  </div>
                </TableHead>
            ...
            <TableBody>
              {paginatedRequests.map((request, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{request.lecturer.fullName}</TableCell>
                  <TableCell>{request.lecturer.email}</TableCell>
                  <TableCell>
                    {" "}
                    Slot {request.oldSlot}-{format(request.oldDate, "dd/MM/yyyy")}
                  </TableCell>
                  <TableCell>
                    {" "}
                    Slot {request.newSlot}-{format(request.newDate, "dd/MM/yyyy")}
                  </TableCell>
                  <TableCell>
