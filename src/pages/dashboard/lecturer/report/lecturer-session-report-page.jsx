...
import { Loader2, ArrowLeft, FileScanIcon as FileAnalytics, Eye } from "lucide-react";
...
export default function ViewLecturerClassReportPage() {
      ...
      reports: [
        {
          ...
          status: 2,
          summary: "This session covered Entity Framework Core fundamentals including DbContext, migrations, and CRUD operations. Students demonstrated good understanding of the concepts.",
        },
      ],
    },
  ]);
                ...
                reports: [
                  {
                    reportId: `${Math.random().toString(36).substring(2, 15)}`,
                    status: 2,
                    summary: `Generated report for session ${session.sessionNumber}: ${session.description}. The session was successful with all students attending.`,
                  },
                ],
              };
            }
  ...
  function getActionButton(session) {
    if (!session.reports) {
      return (
        <Button onClick={() => handleAnalyze(session.sessionId)} disabled={processingSessionId === session.sessionId} size="sm">
      ...
      return (
        <Button variant="outline" size="sm" onClick={() => alert(session.reports[0].summary)}>
