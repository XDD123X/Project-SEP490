          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0">
              <Calendar
                ...
                onSelect={(date) => date && setSelectedWeek(startOfWeek(date))}
                initialFocus
              />
            </PopoverContent>
          </Popover>
    ...
    <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        ...
        <tbody>
          {timeSlots.map((slot) => (
            <tr key={slot.id}>
              <td className="p-2 border dark:border-gray-700 font-medium">{slot.time}</td>
              {weekDays.map((day) => {
                ...
                return (
                  <td key={`${day.toISOString()}-${slot.id}`} className="p-2 border dark:border-gray-700">
                    {session ? (
                      ...
                      >
                        <CardContent className="p-3 h-full flex flex-col justify-between">
                          <div className="flex flex-col gap-1">
                            ...
                            <div className="text-sm text-muted-foreground">Teacher: {session.teacher}</div>
                          </div>
                          <Badge
                            variant={
                      ...
                      <div className="h-full flex items-center justify-center text-muted-foreground">No class</div>
                    )}
                  </td>
                )
              })}
                      ...
                      <CardContent className="p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            {session.note && (
                              <div className="text-sm mt-1 text-muted-foreground">Note: {session.note}</div>
                            )}
                          </div>
                          <Badge
                            variant={
