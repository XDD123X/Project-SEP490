...
export function NotificationDropdown() {
  ...
  const recentNotifications = messages.slice(0, 5);

  // Unread
  const getUnreadCount = () => {
    if (!notifications["common"] || !readStatus) return 0;
    return notifications["common"].filter((n) => !isRead(n.notificationId)).length;
  };
    ...
    <HoverCard openDelay={100} closeDelay={200}>
      <HoverCardTrigger asChild>
        <Button variant="ghost" size="icon" className={cn("relative", pathname === "/notifications" && "bg-accent")}>
          ...
          {getUnreadCount() > 0 && <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 text-xs bg-red-500">{getUnreadCount()}</Badge>}
        </Button>
      </HoverCardTrigger>
      <HoverCardContent className="w-80 p-0" align="end">
        <div className="p-2 border-b">
          <h4 className="font-medium">Recent Notifications</h4>
          <p className="text-xs text-muted-foreground">{getUnreadCount()} unread notifications</p>
        </div>
          ...
          {recentNotifications.length === 0 ? (
            ...
            <div className="divide-y">
              {recentNotifications.map((message) => (
                <Link key={message.notificationId} to={`/notification/common/${message.notificationId}`} className={cn("block p-3 hover:bg-accent/50 transition-colors", isRead(message.notificationId) && "bg-accent/20")}>
                  ...
                  />
                  <div className="text-xs mt-1 text-muted-foreground mt-3">By: {message.createdByNavigation?.fullName || "N/A"}</div>
                </Link>
              ))}
            </div>
          )}
        ...
        <div className="p-2 border-t">
          <Link to="/notification/common" className="block text-center text-sm text-primary hover:underline">
